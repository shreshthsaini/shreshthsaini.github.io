...
- When a post has a companion repository of Shreshth's own (code, playbook, project page), link it in the hero as a `.hero-repo` row directly under the `.meta` tag row: GitHub mark SVG, the bare `github.com/owner/repo` as link text, then a muted note naming what it is. Copy the block from `blogs/gpu-infra-for-video-gen.html`. Posts without such a repository (technical notes, pointer notebooks, personal essays) get no row, and it never points at a third-party repo.

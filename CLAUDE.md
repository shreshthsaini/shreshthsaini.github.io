...
- When a post has resources of Shreshth's own behind it, list every one in a `.hero-links` row directly under the `.meta` tag row: pill-outline links, each an inline icon plus a short label, in the order project page, code, paper. Labels are what the reader clicks for ("Project page", "Code", "Paper (arXiv)"), never a bare URL. Icons are globe for a page, GitHub mark for a repository, document for a paper. Copy the block from `blogs/rectified-cfgpp.html` (three links) or `blogs/gpu-infra-for-video-gen.html` (one). Posts with no such resources (technical notes, pointer notebooks, personal essays) get no row, and the row never points at a third-party repo. Body references and the citation list stay as they are; this row is the reader's shortcut, not a replacement.
